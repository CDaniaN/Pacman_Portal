...
class Inky(Ghost):
    inky_images = [pg.transform.rotozoom(pg.image.load(f'images/Inky{n + 3}.png'), 0, 2) for n in range(2)]
    ...
    def __init__(self, game):
        super().__init__(game)
        self.timer_normal = Timer(frames=self.inky_images, wait=200)
        ...
        self.image = self.inky_images[0]

        self.timer = self.timer_normal

        self.sound = Sound("sounds/bgm.wav")

    def scatter(self): self.set_target(self.settings.scatter_inky)

    def aggro(self):
        diffX = self.game.blinky.posn.x - (self.game.pacman.posn + self.game.pacman.vel * 2).x
        diffY = self.game.blinky.posn.y - (self.game.pacman.posn + self.game.pacman.vel * 2).y
        self.set_target((self.game.pacman.posn + self.game.pacman.vel * 2) - Vector(diffX, diffY))
    ...
    def update(self, ticks):
        ...
        if self.isPacmanDead: self.vel = Vector()
            ...
            if self.posn.x == self.center_blinky().x and self.posn.y == self.center_blinky().y:
                self.isEaten = False
                self.isScared = False
                self.sound.ghost_stop_scare()
                self.sound.ghost_chase()
                self.timer = self.timer_normal
        ...
        if ticks % 24 == 0:
            if self.isEaten:
                ...
                self.pxPosn += (self.vel * self.settings.speed_inky)
