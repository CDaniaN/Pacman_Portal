...
class Pinky(Ghost):
    pinky_images = [pg.transform.rotozoom(pg.image.load(f'images/Pinky{n + 3}.png'), 0, 2) for n in range(2)]
    ...
    def __init__(self, game):
        super().__init__(game)
        self.timer_normal = Timer(frames=self.pinky_images, wait=200)
        ...
        self.image = self.pinky_images[0]

        self.timer = self.timer_normal

    def scatter(self): self.set_target(self.settings.scatter_pinky)

    def aggro(self): self.set_target(self.game.pacman.posn + self.game.pacman.vel * 4)
    ...
    def update(self, ticks):
        ...
        if ticks % 24 == 0:
            if self.isEaten:
                ...
                self.pxPosn += (self.vel * self.settings.speed_pinky)
